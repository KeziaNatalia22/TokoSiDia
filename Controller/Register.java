...
public class Register {
    ...
    // public static void inputDatatoDB(String username, String phoneNum, String
    // email, String password, String address, String type){
    // String query = "INSERT INTO user (username, phone_number, email, passwrd,
    // address, acc_stat, type)" +
    // "VALUES (?, ?, ?, ?, ?, 'ACTIVE', ?)";
    // try{
    // PreparedStatement st = DatabaseHandler.connect().prepareStatement(query);
    // st.setString(1, username);
    ...
    // JOptionPane.showMessageDialog(null, "Registrasi Berhasil", "Registrasi",
    // JOptionPane.INFORMATION_MESSAGE);
    // } catch (Exception e) {
    // JOptionPane.showMessageDialog(null, "Error in Input Data", "Error",
    // JOptionPane.ERROR_MESSAGE);
    // }
    // }
    ...
    public static boolean updateData(String username, String phoneNum, String email, String cityLocated,
            String shopName) {
        try {
            String query = "UPDATE user u INNER JOIN toko t ON u.username = t.username " +
            "SET u.phone_number = ?, u.email = ?, t.city_located = ?, t.shop_name = ? " +
            "WHERE u.username = ?";
            PreparedStatement st = DatabaseHandler.connect().prepareStatement(query);
